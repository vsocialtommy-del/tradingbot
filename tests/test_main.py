...
# --------------------------------------------------------------------------- #
# PR #56 (partial): zone freshness window now applies only to dedup.
# `_zone_already_used` blocks new setups on overlapping CONSUMED /
# VIOLATED / FLIPPED zones whose `created_at` is within
# `zone_freshness_hours`. Older "burnt" zones don't lock placement
# out of their price band — solves the production graveyard problem
# (87 dead zones blocking every fresh pattern in a 250-point band).
#
# PR #56's loader half (`_load_confirmed_candidates` freshness) was
# tied to PR #55 and was removed when PR #55 was reverted.
# --------------------------------------------------------------------------- #
